...
async function handleCommands(message, client) {
    const prefix = 'X';
    // Match both standard mention <@id> and nickname mention <@!id>
    ...
    // NEW: If user ONLY mentions the bot, tell them the prefix
    if (mentionRegex.test(message.content) && message.content.trim() === usedPrefix) {
        return message.reply(`My prefix is \`${prefix}\`. Try \`${prefix} help\`!`);
    }
    ...
    if (command) {
        try {
            // Note: Prefix commands use (message, args, client)
...
async function handleLeveling(message, client) {
    // ... (Keep your existing handleLeveling code exactly as it is)
    try {
